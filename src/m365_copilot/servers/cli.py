...
def main():
    ...
    if not TENANT_ID or not USER_OID:
        ...
        print("  2. 打开 DevTools → Console, 运行:")
        print("     fetch('https://graph.microsoft.com/v1.0/me').then(r=>r.json()).then(d=>console.log(d.id))")
        print("     fetch('https://graph.microsoft.com/v1.0/organization').then(r=>r.json()).then(d=>console.log(d.value[0].id))")
        print()
        print("然后写入 .env 文件:")
